...
class Experiment:
    def __init__(self,eeg,michael  = False ,exp_num = None):
        """
    ...
    def _init_labels(self):
        ...
        for i in range(self.num_blocks):
            ...
            while len(self.labels[i]) < self.num_trials: 
                Idle_sequence_length = random.randint(3,5)
    ...
    def success_screen(self):
        ...
        image = visual.ImageStim(win, image='Pictures/sucess.png',size=[2, 2])  
        image.contrast = 0.5
    ...
    def ask_counted_target(self,i): 
        ...
        """
        def get_num_target_ent(event):
            return get_num_target(entry.get())

        def get_num_target(input=None):
